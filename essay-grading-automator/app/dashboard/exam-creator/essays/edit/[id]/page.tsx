...
export default function EditEssayPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const fetchEssay = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/essays/${params.id}`)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Edit Essay</h1>
        <p className="text-muted-foreground">Update essay template and grading criteria</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Edit the essay question and grading criteria</CardDescription>
          </CardHeader>
